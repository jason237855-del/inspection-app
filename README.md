...
- 2026-09-07：把 `package-lock.json` 加入版控（先前一直是 untracked 狀態）。原因：確保新舊兩台電腦、以及未來任何人 `npm install` 時安裝到的套件版本完全一致，避免環境不一致造成的問題。

- 2026-09-06：報告表格的紅字範圍縮小成只有「結果」欄位變紅，編號／空間／部位／檢測項目欄位維持原本顏色（先前是整列文字都變紅）。
